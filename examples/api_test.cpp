...
struct example_two{
  example_two(){
    stdexec::sender auto my_sender = stdexec::schedule(stdexx::qthreads_scheduler{});
    stdexec::sender auto algorithm = stdexec::then(my_sender, [](int arg) {return 1;}); 
      
      auto val =
        stdexec::sync_wait(algorithm).value();
      std::cout << std::get<0>(val) << std::endl;
  }
};
...
struct example_three{
  ...
  stdexec::sender auto s =
      stdexec::schedule(stdexx::qthreads_scheduler{}) | stdexec::then([](int arg) {
        std::cout << "Hello from user-level thread in then-functor!" << std::endl;
      }) |
      stdexec::bulk(stdexec::par, 20, [](int i) {
        std::cout << "Hello from user-level thread in bulk!(i=" << i << ")" << std::endl;
      });
...
auto main() -> int {
  stdexx::init(); 
  using types = std::variant<example_one, example_two, example_three, example_four>;
  std::vector<types> v = {{},{},{},{},{}};
  stdexx::finalize();
  return 0;
}
